...
<div id="rfqpoPage" class="rfqpo-shell cl-form-page cl-list-host-page">
<div class="card cl-document-form">
  <div class="card-body">
  ...
  <div class="d-flex justify-content-between align-items-center mb-2">
    ...
    <button type="button" id="manageTagsBtn" class="btn btn-sm btn-outline-secondary btn-manage-tags" data-doc-type="" data-doc-id="" disabled>
      <i class="bi bi-tags me-1"></i>Manage Tags
    </button>
  </div>
  <form id="rfqpoForm" class="mb-4 cl-form-page">
      ...
      <style>
        ...
        #rfqpoPage .cl-list-page {
          padding-top: .4rem !important;
          padding-bottom: .75rem !important;
        }
        #listWrapper { margin-top: 0 !important; padding-top: 0 !important; }
        .rfqpo-list-top {
          display: flex;
      ...
      <div class="d-flex justify-content-end gap-2 mt-2">
        ...
        <button type="button" id="convertRfqFormBtn" class="btn btn-success btn-sm" style="display:none"><i class="bi bi-arrow-right-circle me-1"></i>Convert to PO</button>
    ...
    <div id="listWrapper" class="cl-list-page cl-products-page">
      <div class="cl-list-page-header">
        <div>
          <h2 class="mb-0"><i class="bi bi-cart3 me-2"></i>RFQ / Purchase Orders</h2>
          <div class="small text-muted">Request for Quotations and confirmed Purchase Orders</div>
        </div>
        <div class="cl-list-page-actions">
          <button id="refreshList" class="btn btn-outline-secondary cl-action-icon" title="Refresh" aria-label="Refresh">
            <i class="bi bi-arrow-clockwise"></i>
          </button>
          <button id="showFormBtn" class="btn btn-primary cl-products-new-btn"><i class="bi bi-plus-circle me-2"></i>New RFQ</button>
        </div>
      </div>

      <div class="pl-tabs-bar cl-list-tabs" id="listTabs">
        <a href="#" class="pl-tab-btn active" data-filter="all">All <span class="count-badge cl-count-badge ms-1"></span></a>
        <a href="#" class="pl-tab-btn" data-filter="open">Open <span class="count-badge cl-count-badge ms-1"></span></a>
        <a href="#" class="pl-tab-btn" data-filter="partial">Partial Received <span class="count-badge cl-count-badge ms-1"></span></a>
        <a href="#" class="pl-tab-btn" data-filter="late">Late PO <span class="count-badge cl-count-badge ms-1"></span></a>
        <a href="#" class="pl-tab-btn" data-filter="closed">Completed <span class="count-badge cl-count-badge ms-1"></span></a>
        <a href="#" class="pl-tab-btn" data-filter="cancelled">Cancelled <span class="count-badge cl-count-badge ms-1"></span></a>
      </div>

      <form class="cl-list-toolbar cl-products-toolbar" onsubmit="return false;">
        <div class="cl-list-search-group">
          <label class="form-label" for="rfqpoSearch">Search</label>
          <div class="search-input-wrapper">
            <i class="bi bi-search search-icon"></i>
            <input id="rfqpoSearch" class="form-control cl-list-search" placeholder="Search PO #, vendor, product…" autocomplete="off" />
          </div>
        </div>
        <div class="cl-list-toolbar-controls">
          <button id="searchListBtn" class="btn btn-outline-secondary cl-action-icon" title="Search purchase documents" aria-label="Search purchase documents">
            <i class="bi bi-search"></i>
          </button>
          <button id="clearListBtn" class="btn btn-outline-secondary cl-action-icon" aria-label="Clear">
...
(() => {
  ...
  data.tax_amount = parseFloat(totals.totalTax.toFixed(2));
    ...
    try {
      ...
      // Enable/assign Manage Tags button for this document (RFQ or PO)
      try {
        const assignedId = (newId || rfqId) || form.rfq_id.value || null;
        const docTypeForTags = isEditingPo ? 'purchase_order' : 'purchase_rfq';
        if (assignedId) setManageTagsButton(docTypeForTags, assignedId);
      } catch (e) {}
  ...
  function renderCombinedWithPagination(paginationData){
    ...
    if(!rows || rows.length===0){ combinedList.innerHTML = '<div class="cl-list-empty"><span class="cl-list-empty__icon" aria-hidden="true"><i class="bi bi-cart3"></i></span><h6>No Purchase Documents Found</h6><p>RFQs and purchase orders matching this view will appear here.</p></div>'; return; }

    let html = '<table class="cl-table table table-hover pl-table pl-products-table data-table-table mb-0">';
    html += '<colgroup>';
    html += '<col style="width: 34px;">';
    html += '<col style="width: 132px;">';
    html += '<col style="width: 184px;">';
    html += '<col style="width: 130px;">';
    html += '<col style="width: 360px;">';
    html += '<col style="width: 114px;">';
    html += '<col style="width: 114px;">';
    html += '<col style="width: 96px;">';
    html += '<col style="width: 134px;">';
    ...
    html += '<th style="width:12%">Tags</th>';
    ...
    rows.forEach((r, idx) => {
      ...
      html += `<td><a href="${viewUrl}" class="cl-code-link" onclick="event.stopPropagation()"><code>${displayNum}</code></a></td>`;
      // Vendor
      html += `<td><div class="pl-name text-decoration-none">${escHtml(r.vendor_name || '—')}</div></td>`;
      // Tags placeholder (will be populated via AJAX)
      const docType = (r.type === 'RFQ') ? 'purchase_rfq' : 'purchase_order';
      const docId = r.id || r.po_id || r.rfq_id || '';
      html += `<td class="doc-tags" data-doc-type="${docType}" data-doc-id="${docId}"><em style="font-size:.72rem;color:var(--cl-text-muted)">Loading…</em></td>`;
      ...
      if (spName || displayCode) {
        const nameLine = spName ? `<div class="pl-name">${escHtml(spName)}</div>` : '';
        let subParts = [];
        if (displayCode) subParts.push(`<span class="pl-desc">${escHtml(displayCode)}</span>`);
      ...
      html += `<td class="cl-data-number" style="text-align:right;white-space:nowrap">${formatTotal(totalVal, rowCurrency)}</td>`;
      
      // Status badges
      let statusHtml = '';
      if (r.type === 'RFQ') {
        const rfqState = (r.state || 'draft').toLowerCase();
        statusHtml = `<span class="cl-status-badge cl-status-badge--info"><span class="cl-status-badge__dot"></span>RFQ</span> `;
        if (rfqState === 'draft') statusHtml += `<span class="cl-status-badge cl-status-badge--warning"><span class="cl-status-badge__dot"></span>Draft</span>`;
        else if (rfqState === 'sent') statusHtml += `<span class="cl-status-badge cl-status-badge--info"><span class="cl-status-badge__dot"></span>Sent</span>`;
        else if (rfqState === 'cancelled') statusHtml += `<span class="cl-status-badge cl-status-badge--danger"><span class="cl-status-badge__dot"></span>Cancelled</span>`;
      } else {
        const recStatus = r.receipt_status || 'open';
        statusHtml = `<span class="cl-status-badge cl-status-badge--info"><span class="cl-status-badge__dot"></span>PO</span> `;
        if (recStatus === 'open') statusHtml += `<span class="cl-status-badge cl-status-badge--info"><span class="cl-status-badge__dot"></span>Open</span>`;
        else if (recStatus === 'partial_received') statusHtml += `<span class="cl-status-badge cl-status-badge--warning"><span class="cl-status-badge__dot"></span>Partial</span>`;
        else if (recStatus === 'fully_received') statusHtml += `<span class="cl-status-badge cl-status-badge--success"><span class="cl-status-badge__dot"></span>Received</span>`;
        else if (recStatus === 'closed') statusHtml += `<span class="cl-status-badge cl-status-badge--neutral"><span class="cl-status-badge__dot"></span>Closed</span>`;
        else if (recStatus === 'cancelled') statusHtml += `<span class="cl-status-badge cl-status-badge--danger"><span class="cl-status-badge__dot"></span>Cancelled</span>`;
      }
      html += `<td>${statusHtml}</td>`;
      
      // Actions
      html += `<td style="text-align:right;white-space:nowrap;overflow:visible" onclick="event.stopPropagation()">`;
      html += `<a href="${viewUrl}" class="pl-list-act cl-icon-action" title="View" onclick="event.stopPropagation()"><i class="bi bi-eye"></i></a>`;
      // Manage Tags button (opens shared tag modal)
      html += `<button type="button" class="pl-list-act cl-icon-action ms-1 btn-manage-tags" data-doc-type="${docType}" data-doc-id="${docId}" title="Manage Tags" onclick="event.stopPropagation()"><i class="bi bi-tags"></i></button>`;
    ...
    wireUpTableEvents();
    // Load tags for each rendered row (fills .doc-tags cells)
    (function loadTagsForList(){
      try {
        document.querySelectorAll('.doc-tags').forEach(function(td){
          try {
            const dt = td.dataset.docType || '';
            const id = td.dataset.docId || '';
            if (!dt || !id) { td.innerHTML = '<span class="text-muted">—</span>'; return; }
            fetch('<?= site_url('document-tags') ?>?document_type=' + encodeURIComponent(dt) + '&document_id=' + encodeURIComponent(id))
              .then(r => r.json())
              .then(j => {
                if (!j || !j.success) { td.innerHTML = '<span class="text-muted">—</span>'; return; }
                td.innerHTML = '';
                if (!j.data || j.data.length === 0) { td.innerHTML = '<span class="text-muted" style="font-size:.72rem">No tags</span>'; return; }
                j.data.forEach(function(t){
                  const sp = document.createElement('span');
                  sp.className = 'badge bg-light text-dark me-1';
                  sp.style.fontSize = '0.72rem';
                  sp.textContent = t.name;
                  td.appendChild(sp);
                });
              }).catch(()=>{ td.innerHTML = '<span class="text-muted">—</span>'; });
  ...
  function wireUpTableEvents(){
    ...
    combinedList.addEventListener('click', async function(ev){
      ...
      try {
        ...
        let tbl = '<table class="cl-table table table-sm mb-0" style="font-size:.76rem"><thead><tr>'
    ...
    combinedList.querySelectorAll('.convertRfqBtn').forEach(b => b.addEventListener('click', ()=> convertRfqToPo(b.dataset.id)));
  ...
  // Convert an RFQ to a PO (shared by the list row action and the edit form button).
  async function convertRfqToPo(id, onDone){
  ...
  // Convert button lives on the edit form too: visible only for a saved RFQ (not a PO).
  function toggleConvertFormBtn(){
    const btn = document.getElementById('convertRfqFormBtn');
    if (!btn) return;
    const form = document.getElementById('rfqpoForm');
    const savedId = form && form.rfq_id ? (form.rfq_id.value || '') : '';
    const isPo = !!new URLSearchParams(window.location.search).get('edit_po');
    btn.style.display = (savedId && !isPo) ? 'inline-block' : 'none';
    btn.dataset.id = savedId;
  }
  document.getElementById('convertRfqFormBtn').addEventListener('click', function(){
    const id = this.dataset.id;
    if (!id) return;
    convertRfqToPo(id, () => {
      document.getElementById('formWrapper').style.display = 'none';
      const lw = document.getElementById('listWrapper');
      if (lw) { lw.style.display = 'block'; lw.scrollIntoView({ behavior:'smooth', block:'start' }); }
    });
  });
  ...
  // Manage Tags button (opens shared tag modal). Enabled when a document id is present.
  const manageTagsBtn = document.getElementById('manageTagsBtn');
  function setManageTagsButton(docType, docId){
    if (!manageTagsBtn) return;
    if (!docType || !docId) {
      manageTagsBtn.dataset.docType = '';
      manageTagsBtn.dataset.docId = '';
      manageTagsBtn.disabled = true;
      return;
    }
    manageTagsBtn.dataset.docType = docType;
    manageTagsBtn.dataset.docId = docId;
    manageTagsBtn.disabled = false;
  }
  ...
  (async () => {
    ...
    try {
      ...
      // enable manage tags for this RFQ
      try { setManageTagsButton('purchase_rfq', form.rfq_id.value || j.data.id); } catch (e) {}
      linesContainer.innerHTML = '';
      (j.data.lines || []).forEach((ln, idx) => {
        addLineRow();
        const row = linesContainer.children[idx];
  ...
  (async () => {
    ...
    try {
      ...
      // enable manage tags for this PO
      try { setManageTagsButton('purchase_order', form.rfq_id.value || po.id); } catch (e) {}
...
(() => {
  ...
  async function openPoModal(id, editable){
    ...
    try {
      ...
      html += '<div class="table-responsive"><table class="cl-table table table-sm"><thead><tr>';
      if (editable) {
  ...
  html += `<div class="table-responsive"><table class="cl-table table table-sm"><thead><tr><th>Code</th><th>Desc</th><th>Qty</th><th>Unit</th><th>Disc %</th><th>Tax %</th><th class="text-end">Line Total</th></tr></thead><tbody>`;
