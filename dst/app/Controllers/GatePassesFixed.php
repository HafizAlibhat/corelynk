...
class GatePassesFixed extends BaseController
{
    ...
    public function updateStatus($id)
    {
        ...
        $gatePass = $this->gatePassModel->findByPublicIdOrId($id);
        if (!$gatePass) {
    ...
    {
        $record = $this->gatePassModel->findByPublicIdOrId($id);
        if (!$record) {
            throw new \CodeIgniter\Exceptions\PageNotFoundException('Gate pass not found');
        }
        $numericId = (int)$record['id'];
        $gatePass = $this->gatePassModel->select("gate_passes.*, vendors.name as vendor_name, vendors.address as vendor_address, vendors.contact_person, vendors.phone as vendor_phone, users.username as created_by_name")
            ->join('vendors', 'vendors.id = gate_passes.vendor_id', 'left')
            ->join('users', 'users.id = gate_passes.created_by', 'left')
            ->find($numericId);
    ...
    public function show($id)
    {
        ...
        $numericId = (int)$record['id'];
        $gatePass = $this->gatePassModel->select("gate_passes.*, vendors.name as vendor_name, vendors.contact_person, vendors.phone as vendor_phone, users.username as created_by_name")
            ->join('vendors', 'vendors.id = gate_passes.vendor_id', 'left')
            ->join('users', 'users.id = gate_passes.created_by', 'left')
            ->find($numericId);
