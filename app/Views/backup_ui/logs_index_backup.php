...
<style>
    ...
    .tt-caret { display:inline-flex; align-items:center; justify-content:center; width:1.1rem; height:1.1rem; line-height:1; font-size:.85rem; color:#495057; border:1px solid #d0d7de; border-radius:3px; background:#fff; margin-right:.35rem; cursor:pointer; }
    ...
    .tt-indent-4 { padding-left: 7rem; padding-right: 1rem; }
    ...
    .tt-actions .btn { padding: .12rem .4rem; font-size: .65rem; }
    ...
    .tree-table tbody tr.log-entry td { background:#0f172a0a; color:#334155; font-weight:500; }
    .tree-table tbody tr.log-entry:hover td { background:#0f172a15; }
    body.theme-dark .tree-table tbody tr.log-entry td { background:#0b1220; color:#cbd5e1; }
    body.theme-dark .tree-table tbody tr.log-entry:hover td { background:#0e1626; }
    ...
    .tree-table tbody tr[data-level="4"] td:first-child .tt-meta::before { content:"🧾 "; }
     ...
     /* Action/button sizing centralized in public/assets/css/style.css. Per-page legacy classes kept but visual sizing
         and icon-only behaviour are controlled by the central stylesheet to ensure consistent UI. */
    ...
    /* Solid orange row hover treatment (applies to key rows) */
    .tree-table tbody tr.batch-row:hover td,
    .tree-table tbody tr.log-entry:hover td {
        background: var(--row-hover-orange) !important;
        color: var(--row-hover-orange-text) !important;
    }
    .tree-table tbody tr.batch-row:hover a.tt-toggle,
    .tree-table tbody tr.log-entry:hover a.tt-toggle,
    .tree-table tbody tr.batch-row:hover .tt-icon,
    .tree-table tbody tr.log-entry:hover .tt-icon,
    .tree-table tbody tr.batch-row:hover .tt-meta,
    .tree-table tbody tr.log-entry:hover .tt-meta {
        color: var(--row-hover-orange-text) !important;
    }
    .tree-table tbody tr.batch-row:hover .tt-badge:not([data-type]),
    .tree-table tbody tr.log-entry:hover .tt-badge:not([data-type]) { background: rgba(255,255,255,.18); border-color: rgba(255,255,255,.32); color: #fff; }
    .tree-table tbody tr.batch-row:hover .tt-status,
    .tree-table tbody tr.log-entry:hover .tt-status { background: rgba(255,255,255,.15); border-color: rgba(255,255,255,.28); color: #fff; }
    /* Focus and active states mirror hover for accessibility */
    .tree-table tbody tr.is-active td,
    .tree-table tbody tr:focus-within td {
        background: var(--row-hover-orange) !important;
        color: var(--row-hover-orange-text) !important;
    }
    .tree-table tbody tr:focus-within td { outline: 2px solid rgba(255,255,255,.18); outline-offset: -2px; }
    .tree-table tbody tr.is-active a.tt-toggle,
    .tree-table tbody tr:focus-within a.tt-toggle,
    .tree-table tbody tr.is-active .tt-icon,
    .tree-table tbody tr:focus-within .tt-icon,
    .tree-table tbody tr.is-active .tt-meta,
    .tree-table tbody tr:focus-within .tt-meta { color: var(--row-hover-orange-text) !important; }
    .tree-table tbody tr.is-active .tt-badge:not([data-type]),
    .tree-table tbody tr:focus-within .tt-badge:not([data-type]) { background: rgba(255,255,255,.18); border-color: rgba(255,255,255,.32); color: #fff; }
    .tree-table tbody tr.is-active .tt-status,
    .tree-table tbody tr:focus-within .tt-status { background: rgba(255,255,255,.15); border-color: rgba(255,255,255,.28); color: #fff; }
...
<div class="card shadow-sm mb-4">
    <div class="card-header start-batch-header d-flex justify-content-between align-items-center">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                ...
                <div class="sbh-sub small">Select Work Order → Product → Process → Vendor</div>
            </div>
        </div>
            ...
            <div id="startLogSection" class="col-12" style="display:none;">
                <div class="row g-2 align-items-end">
                    ...
                    <div class="col-12 d-flex flex-wrap gap-2 align-items-center">
                        ...
                        <span class="text-muted small">(Copy fills qty/date/time; Complete sets Accepted=Planned and zeroes rejects/rework)</span>
                        <div class="flex-fill"></div>
                    </div>
                    <div class="col-12">
                        <small class="text-muted">Copy from batch will fill Received with Planned Qty and set Log Date/Time from Start Date/Time. "Complete now" also fills Accepted and zeroes rejections/rework.</small>
                    </div>
                </div>
            </div>
    ...
    els.process.disabled = true; els.vendor.innerHTML = '<option value="">-- select process first --</option>'; els.vendor.disabled = true; els.vendorHint.textContent=''; els.qty.value = ''; els.qty.disabled = true; els.startDate.value=''; els.startDate.disabled=true;
    ...
    els.startBtn.disabled = !(els.workOrder.value && els.product.value && els.process.value && q > 0 && hasDate && hasTime && inlineOk);
...
async function loadTree() {
        ...
        (wo.products||[]).forEach((p, pidx, parr) => {
            ...
            (p.processes||[]).forEach((pr, ridx, rarr) => {
                ...
                const targetBadge = targetVal
                    ? `<span class=\"tt-badge\">Target: ${targetVal} / ${acceptedVal}</span>`
                    : '';
                trR.innerHTML = `
                ...
                (pr.batches||[]).forEach((b, bidx, barr) => {
                    ...
                    trB.innerHTML = `
                        <td class="tt-indent-3">
                            ...
                            <span class=\"tt-inline\">
                                ...
                                <span class=\"tt-badge slim\" data-type=\"pending\">Pending: ${fmt(pendingVal) || 0}</span>
                            </span>
                        </td>
    ...
    table.addEventListener('click', (e)=>{
        ...
        if (expanded) { collapseFrom(tr); caret.textContent = '+'; }
        else { expandImmediate(tr); caret.textContent = '−'; }
        e.preventDefault();
    });
    ...
    document.getElementById('ttCollapseAll').addEventListener('click', () => {
        table.querySelectorAll('tr.tt-row[data-level="0"], tr.tt-row[data-level="1"], tr.tt-row[data-level="2"], tr.tt-row[data-level="3"]').forEach(tr => { collapseFrom(tr); const c = tr.querySelector('.tt-caret'); if (c) c.textContent = '+'; }); });
...
// Load vendors for selected process
    ...
    const isVendorProc = !!res.is_vendor_process;
